...
namespace rev {
	namespace test {
		class FileTreeTest : public Random {};
		...
		TEST_F(FileTreeTest, Difference) {
			...
			class History {
				public:
					using FEvS = std::unordered_map<const FEv*, FEv_UP, FEv_hash, FEv_equal>;
					FEvS	_hist;
				...
				public:
					...
					void addEvent(FEv* e) {
						...
						auto res = _hist.emplace(e, FEv_UP(e));
						if(res.second)
							_info[path].emplace_back(e);
						else
							Assert0(_info.count(path) > 0);
					}
					//! 関連イベントを全て消す
					/*! \param[in] path フルパス */
					void removeEvent(const Dir& path) {
						...
						if(itr != _info.end()) {
							Assert0(!itr->second.empty());
							for(auto* ev : itr->second) {
								auto itr2 = _hist.find(ev);
								Assert0(itr2!=_hist.end());
								_hist.erase(itr2);
							}
							_info.erase(itr);
						}
			...
			struct Ntf : ActionNotify {
				...
				String_SP			_basePath;
				const int			_nseg;
				Ntf(History& hist, const String_SP& basePath, int nseg, StringSet& nameset, StringSet& orig_pathset):
					_hist(hist),
					_nameset(nameset),
				...
				void onMove_Pre(const Dir& from, const Dir& to) override {
					const std::string pathFrom = from.getSegment_utf8(_nseg, Dir::End),
									pathTo = to.getSegment_utf8(_nseg, Dir::End);
					const bool bDir = from.isDirectory();
			...
			const auto basePath = std::make_shared<std::string>(dir.plain_utf8());
			Ntf ntf(hist,
					basePath,
					dir.segments(),
					nameset,
			...
			// 変更が検出できてるかチェック
			const File_UP ft1 = FileTree::ConstructFTree(dir);
			...
			struct MyVisitor : FRecvNotify {
				...
				void check(const FEv_UP& e) {
					const auto itr = _hist._hist.find(e.get());
					EXPECT_NE(itr, _hist._hist.end());
					_hist._hist.erase(itr);
				}

				#define DEF_CHECK(typ)	void event(const typ& e, const Data_SP& /*ud*/) override { check(FEv_UP(new typ(e))); }
