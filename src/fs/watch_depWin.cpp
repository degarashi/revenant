...
namespace rev {
	...
	FNotify_depWin::Entry::Entry(Entry&& e):
		hDir(std::move(e.hDir)),
		hEvent(std::move(e.hEvent)),
		ovl(e.ovl),
		basePath(std::move(e.basePath))
	{
	...
	void FNotify_depWin::_SetWaitNotify(Entry& ent) {
		...
		const BOOL b = ReadDirectoryChangesW(ent.hDir.get(),
						ent.buff,
	...
	void FNotify_depWin::procEvent(const CallbackD& cb) {
		_csection.enter();
		for(auto& e : _eventL)
			cb(e);
		_eventL.clear();
	...
	FNotify_depWin::DSC FNotify_depWin::addWatch(const std::string& path, uint32_t) {
		...
		ent.basePath = String_SP(new std::string(path));
	...
	void FNotify_depWin::_pushInfo(Entry& ent, FILE_NOTIFY_INFORMATION& pData, const uint32_t cookie) {
		FileEvent id;
	    switch(pData.Action) {
		...
		// (バイト数)
	    const uint32_t lenBytes = pData.FileNameLength;
