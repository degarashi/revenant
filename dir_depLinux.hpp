...
namespace rev {
	...
	struct Dir_depLinux {
		...
		static FStatus CreateFStatus(const struct stat& st) noexcept;
		//! ファイルフラグ変換 (spinner -> linux)
		static uint32_t ConvertFlag_S2L(uint32_t flag) noexcept;
		//! ファイルフラグ変換 (linux -> spinner)
		static uint32_t ConvertFlag_L2S(uint32_t flag) noexcept;
