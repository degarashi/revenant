...
namespace rev {
	namespace {
		void InsertIndent(std::ostream& os, const int n) {
			for(int i=0 ; i<n ; i++)
				os << '\t';
		}
	}
	...
	// ------------------- FileInfo -------------------
	FileTree::Type FileInfo::getType() const {
		return FileTree::Type::File;
	}
	...
	// ------------------- DirInfo -------------------
	FileTree::Type DirInfo::getType() const {
		return FileTree::Type::Directory;
	}
	bool DirInfo::isDirectory() const {
		return true;
	}
	void DirInfo::print(std::ostream& os, const int indent) const {
		InsertIndent(os, indent);
	...
	// ------------------- FileTree -------------------
	//! 続きから走査
	File_UP FileTree::ConstructFTree(Dir& dir) {
		DirInfo* pd;
		File_UP upd(pd = new DirInfo(dir.getLast_utf8(), dir.status()));
	...
	//! ツリートップから走査
	File_UP FileTree::ConstructFTree(To32Str path) {
		Dir dir(path);
		Assert(dir.status().flag & FStatus::DirectoryType, "path is not directory");
		return ConstructFTree(dir);
	}

	void FileTree::For_Each(const IFTFile* ft, const std::function<void(const std::string&, const IFTFile*)>& cb, PathBlock& pblock) {
		D_Assert0(ft->isDirectory());
	...
	void FileTree::ThrowCreateEvent(FRecvNotify& visitor, const String_SP& base, const IFTFile* ft, PathBlock& pblock) {
		For_Each(ft, [&](const std::string& str, const IFTFile* ft) {
			visitor.event(FEv_Create(base, str, ft->isDirectory()), Data_SP());
		}, pblock);
	}
	void FileTree::ThrowRemoveEvent(FRecvNotify& visitor, const String_SP& base, const IFTFile* ft, PathBlock& pblock) {
		For_Each(ft, [&](const std::string& str, const IFTFile* ft) {
			visitor.event(FEv_Remove(base, str, ft->isDirectory()), Data_SP());
		}, pblock);
	}
	bool FileTree::VisitDifference(FRecvNotify& visitor, const String_SP& base, const IFTFile* f0, const IFTFile* f1, PathBlock& pblock) {
		D_Assert0(f0->isDirectory() && f1->isDirectory());
		...
		for(auto& ent : fd0->fmap) {
			...
			// エントリが削除された時の処理
			auto onRemove = [&](){
				visitor.event(FEv_Remove(base, pblock.plain_utf8(), bDir), Data_SP());
			...
			if(itr == fd1->fmap.end()) {
				...
				if(bDir != bDir2) {
					onRemove();
					visitor.event(FEv_Create(base, path8, bDir2), Data_SP());
					if(bDir2) {
					...
					if(!bDir) {
						if(fs0.ftime.tmAccess != fs1.ftime.tmAccess)
							visitor.event(FEv_Access(base, path8, bDir), Data_SP());
						// ファイル編集時刻チェック => FEv_Modify
						if(fs0.ftime.tmModify != fs1.ftime.tmModify ||
							fs0.ftime.tmCreated != fs1.ftime.tmCreated ||
							fs0.size != fs1.size)
							visitor.event(FEv_Modify(base, path8, bDir), Data_SP());
					}
					// アトリビュート変更チェック => FEv_Attr
					fs0.ftime = fs1.ftime;
					fs0.size = fs1.size;
					if(fs0 != fs1)
						visitor.event(FEv_Attr(base, path8, bDir), Data_SP());
		...
		for(auto& ent : fd1->fmap) {
			...
			if(itr == fd0->fmap.end()) {
				...
				bool bDir = ent.second->isDirectory();
				visitor.event(FEv_Create(base, pblock.plain_utf8(), bDir), Data_SP());
				if(bDir)
		...
		if(bNodeChanged)
			visitor.event(FEv_Modify(base, pblock.plain_utf8(), true), Data_SP());
		return bNodeChanged;
	}
}
